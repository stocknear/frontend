<script lang="ts">
  ...
  import { stock_detail_no_data } from "$lib/paraglide/messages";

  export let processedData: Record<
    string,
    { xList: string[]; valueList: number[]; labelName: string }
  > = {};
  ...
  {:else}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {#each validMetrics as config (config.propertyName)}
        {@const data = processedData[config.propertyName]}
        {#if data}
          {@const ghostLabels = Array.from(
            { length: effectiveGhostCount },
            () => "",
          )}
          {@const ghostVals = buildGhostValues(
            data.valueList,
            effectiveGhostCount,
          )}
          <FinancialChartCard
            ...
            seriesData={[
              {
                ...
                label: config.label,
                values: [...ghostVals, ...data.valueList],
              },
            ]}

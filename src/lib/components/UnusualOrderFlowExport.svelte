<script lang="ts">
  ...
  const getExportFilename = (): string => {
    if (selectedDate) {
      ...
      return `unusual-order-flow-historical_${year}-${month}-${day}.csv`;
    } else {
      // Live data - use today's date
      const today = new Date().toISOString().split("T")[0];
      return `unusual-order-flow-live_${today}.csv`;
    }
  };
  ...
  const convertToCSV = (dataArray: any[]): string => {
    ...
    const columns = [
      ...
      { key: "premium", label: "Avg. Paid" },
      { key: "size", label: "Size" },
      { key: "volume", label: "Volume" },
      { key: "sizeVolRatio", label: "% Size / Vol" },
      { key: "sizeAvgVolRatio", label: "% Size / Avg Vol" },
  ...
  const startExport = async () => {
    if (!data?.user) {
      errorMessage = "Sign in to export unusual order flow data.";
      return;
    }

    if (!isEligible) {
      errorMessage = "Upgrade to Pro to export unusual order flow data.";
      return;
    }
    ...
    try {
      ...
      const response = await fetch("/api/unusual-order-flow-export", {
        method: "POST",
...
<Button
  ...
  title="Download unusual order flow data"
>
...
<input
  type="checkbox"
  id="unusual-order-flow-export-modal"
  class="modal-toggle"
  bind:checked={exportModalOpen}
/>

<dialog id="unusual-order-flow-export-modal" class="modal p-3 sm:p-0">
  <label
    for="unusual-order-flow-export-modal"
  ...
  >
    ...
    >
      Export unusual order flow data
    </h3>
    <p class="mt-2 text-sm leading-relaxed text-gray-600 dark:text-zinc-300">
      Download the current unusual order flow table as a CSV file.
    </p>

<script lang="ts">
  ...
  export let chartOrder: Array<{ propertyName: string; label: string }> | undefined = undefined;
  ...
  // Use chartOrder if provided, otherwise fall back to statementConfig
  $: orderSource = chartOrder ?? statementConfig;

  // Filter out metrics that have no data
  $: validMetrics = orderSource.filter(config => {

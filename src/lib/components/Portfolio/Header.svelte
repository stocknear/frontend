<script lang="ts">
    ...
    export let portfolioName = "Portfolio Tracker";
        ...
        <div class="flex flex-col items-start w-full">
            ...
            >
                <div class="flex flex-col items-start">
                    <h1 class="text-xl sm:text-2xl font-bold">
                        {portfolioName?.length > charNumber
                            ? portfolioName?.slice(0, charNumber) + "..."
                            : portfolioName}
                    </h1>
                </div>
            </div>
            ...
            >
                <div class="w-full max-w-[50%] whitespace-nowrap pr-5">
                    <div class="text-3xl sm:text-4xl font-bold inline">
                        {displayLegend?.close}
                    </div>
                    <div class="font-semibold inline text-xl sm:text-2xl">
                        <span
                    ...
                    <div class="mt-0.5 text-[0.85rem] sm:text-sm">
                        ...
                        {displayLegend?.date}
                        {#if isOpen}
                            <span class="inline mb-0.5 sm:mb-0"
                                >- Market open</span
                            >
                        {/if}
                    </div>

            ...
            <tbody>
                {#each wiim as item}
                    <tr class="border-b border-gray-300 dark:border-zinc-700"
                        ><td
                            class="hidden sm:inline-block pr-2 pt-4 align-top text-xs whitespace-nowrap font-semibold uppercase tracking-wide text-muted dark:text-zinc-300"
                            >{formatDate(item?.date, true)}
                        </td>
                        <td
                            class="py-3 sm:pl-2 leading-6 text-muted dark:text-zinc-200"
                        >
                            <span
                                class="sm:hidden text-xs uppercase tracking-wide text-muted dark:text-zinc-300 font-semibold"
                                >{dashboard_time_ago({
                            ...
                            {#if item?.symbolList && item?.symbolList?.length > 0}
                                {#each item?.symbolList as symbol}
                                    <a
                                        ...
                                        class="inline-flex items-center rounded-full border border-gray-300 shadow dark:border-zinc-700 bg-[#f8fbfb] dark:bg-zinc-900/50 px-2 py-0.5 text-xs font-semibold text-violet-800 dark:text-violet-400 transition sm:hover:text-muted dark:sm:hover:text-white ml-1.5 mb-1"
                                        >{symbol}</a
                                    >
                                {/each}
                            {:else if item?.symbol}

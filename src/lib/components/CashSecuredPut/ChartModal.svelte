<script lang="ts">
  ...
  import {
    cash_secured_put_screener_chart_price_at_expiration,
    cash_secured_put_screener_chart_last_price_label,
    cash_secured_put_screener_chart_breakeven_label,
    cash_secured_put_screener_chart_profit_loss,
    cash_secured_put_screener_chart_tooltip_underlying,
    cash_secured_put_screener_chart_tooltip_profit_loss,
    cash_secured_put_screener_chart_modal_title,
    cash_secured_put_screener_chart_trade_setup,
    cash_secured_put_screener_chart_sell_put,
    cash_secured_put_screener_chart_cash_secured,
    cash_secured_put_screener_chart_net_credit,
    cash_secured_put_screener_chart_breakeven,
    cash_secured_put_screener_chart_below,
    cash_secured_put_screener_chart_last_price,
    cash_secured_put_screener_chart_moneyness,
    cash_secured_put_screener_chart_return_if_flat,
    cash_secured_put_screener_chart_return_if_assigned,
    cash_secured_put_screener_chart_probability_of_profit,
    cash_secured_put_screener_chart_annualized_abbr,
    cash_secured_put_screener_chart_discount,
  ...
  function buildChartConfig(item: any) {
    ...
    const breakeven = item.breakeven; // strike - bid
    ...
    // CSP payoff: sell put at strike, collect premium
    // If s >= strike: profit = premium * 100 (max profit)
    ...
    return {
      ...
      xAxis: {
        ...
        max: xMax,
        title: {
          text: cash_secured_put_screener_chart_price_at_expiration(),
        ...
        plotLines: [
          {
            ...
            width: 1.5,
            label: {
              text: `<span class="text-black dark:text-white text-sm">${cash_secured_put_screener_chart_last_price_label({ price: stockPrice })}</span>`,
          ...
          },
          {
            ...
            width: 1.5,
            label: {
              text: `<span class="hidden sm:block text-sm" style="color:#10B981">${cash_secured_put_screener_chart_breakeven_label({ breakeven: breakeven?.toFixed(2) })}</span>`,
              style: { color: "#10B981" },
            },
            zIndex: 5,
          },
      ...
      yAxis: {
        title: {
          text: `<span class='hidden sm:block'>${cash_secured_put_screener_chart_profit_loss()}</span>`,
          style: { color: $mode === "light" ? "#545454" : "white" },
        },
        gridLineWidth: 1,
        ...
        formatter: function () {
          ...
          return `
            <div class="flex flex-col items-start text-sm">
              <div>
                <span class="font-semibold">${cash_secured_put_screener_chart_tooltip_underlying()}</span> $${underlyingPrice?.toFixed(2)}
                (<span>${pctChange?.toFixed(2)}%</span>)
              </div>
              <div>
                <span class="font-semibold">${cash_secured_put_screener_chart_tooltip_profit_loss()}</span>

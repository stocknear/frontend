<script lang="ts">
  ...
  function computeLocalSR() {
    ...
    const supportLevels = rawData.filter((item) => item.price_level < currentPrice);
    const resistanceLevels = rawData.filter((item) => item.price_level > currentPrice);
    ...
    if (supportLevels.length > 0) {
      ...
      keySupport = {
        ...
        size: strongest.size,
        distancePct: Math.abs(((strongest.price_level - currentPrice) / currentPrice) * 100),
  ...
  function getVolumeChart() {
    ...
    const colors = priceLevel?.map((price, index) => {
      ...
      if (isMax) return isSupport ? "#22c55e" : isResistance ? "#ef4444" : "#71717a";
      if (isSupport) return intensity > 0.5 ? "#4ade80" : $mode === "light" ? "#bbf7d0" : "#166534";
      if (isResistance) return intensity > 0.5 ? "#f87171" : $mode === "light" ? "#fecaca" : "#991b1b";
    ...
    return {
      ...
      xAxis: {
        ...
        plotLines: [
          {
            ...
            label: {
              ...
              style: { color: $mode === "light" ? "#000" : "#fff", fontSize: "10px" },
            },
            zIndex: 5,
          },
        ],
        ...
        labels: {
          style: { color: $mode === "light" ? "#545454" : "white", fontSize: "9px" },
          formatter: function () {
            return "$" + this.value;
          },
        },
      ...
      tooltip: {
        ...
        formatter: function () {
          ...
          const pctOfTotal = localTotalVolume > 0 ? ((volume / localTotalVolume) * 100).toFixed(1) : 0;
          const isSupport = price < currentPrice;
          const isResistance = price > currentPrice;
          const zone = isSupport ? "Support Zone" : isResistance ? "Resistance Zone" : "Current Price";
          const zoneColor = isSupport ? "#22c55e" : isResistance ? "#ef4444" : "#71717a";
      ...
      series: [
        {
          ...
          data: sizes,
          animation: false,
          ...(chartType !== "column" && { color: $mode === "light" ? "#6366f1" : "#818cf8" }),
        },
      ],
    };
  }
  ...
  {#if rawData?.length !== 0 && Object?.keys(metrics)?.length > 0}
    ...
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center gap-1.5">
        <h3 class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">Key Price Levels</h3>
        <InfoModal
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        ...
        <div class="bg-white/70 dark:bg-zinc-950/40 border border-gray-300 dark:border-zinc-700 rounded-2xl p-4">
          <div class="text-sm mb-2 flex items-center">
            ...
            {#if keySupport?.strength}
              <span class="ml-auto text-xs tabular-nums text-gray-400 dark:text-zinc-500">
                Strength: {typeof keySupport.strength === 'number' ? keySupport.strength.toFixed(1) : keySupport.strength}%
              </span>
            {/if}
          </div>
          {#if keySupport}
            <div class="flex items-baseline">
              <span class="text-xl font-semibold tabular-nums text-emerald-600 dark:text-emerald-400">
                ${keySupport.price?.toFixed(2)}
              </span>
            </div>
            <div class="text-sm text-gray-500 dark:text-zinc-400 mt-1 tabular-nums">
              {keySupport.distancePct?.toFixed(1)}% below current · {abbreviateNumber(keySupport.size)} shares
            </div>
          {:else}
        ...
        <div class="bg-white/70 dark:bg-zinc-950/40 border border-gray-300 dark:border-zinc-700 rounded-2xl p-4">
          <div class="text-sm mb-2 flex items-center">
            ...
            {#if keyResistance?.strength}
              <span class="ml-auto text-xs tabular-nums text-gray-400 dark:text-zinc-500">
                Strength: {typeof keyResistance.strength === 'number' ? keyResistance.strength.toFixed(1) : keyResistance.strength}%
              </span>
            {/if}
          </div>
          {#if keyResistance}
            <div class="flex items-baseline">
              <span class="text-xl font-semibold tabular-nums text-red-500 dark:text-red-400">
                ${keyResistance.price?.toFixed(2)}
              </span>
            </div>
            <div class="text-sm text-gray-500 dark:text-zinc-400 mt-1 tabular-nums">
              {keyResistance.distancePct?.toFixed(1)}% above current · {abbreviateNumber(keyResistance.size)} shares
            </div>
          {:else}
            <div class="text-xl font-semibold text-gray-400 dark:text-zinc-600">—</div>
          {/if}
    ...
    <p class="text-xs text-gray-400 dark:text-zinc-500 text-center mt-3">
      Based on 30-day institutional order flow. Direction unknown — use with other analysis.
    </p>

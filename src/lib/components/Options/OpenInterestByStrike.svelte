<script lang="ts">
  ...
  $: columns = [
    {
      key: "strike",
      label: stock_detail_options_oi_col_strike(),
      align: "left",
    },
    {
      key: "call_oi",
...
<div class="sm:pl-7 sm:pb-7 sm:pt-7 w-full m-auto mt-2 sm:mt-0">
  ...
  <p class="mt-3 mb-2 text-sm text-muted dark:text-zinc-300 leading-relaxed">
    {#if rawData?.length > 0}
      ...
      {" "}
      <span
        >{@html stock_detail_options_oi_summary_price({
  ...
  <div class="mt-7 flex flex-wrap items-center justify-between gap-3">
    <DropdownMenu.Root>
      ...
      <DropdownMenu.Content
        ...
        sideOffset={10}
        alignOffset={0}
        class="min-w-56 w-auto max-w-64 max-h-[400px] overflow-y-auto scroller relative rounded-xl border border-gray-300 dark:border-zinc-700 bg-white/95 dark:bg-zinc-950/95 p-2 text-muted dark:text-zinc-200 shadow-none"
      >

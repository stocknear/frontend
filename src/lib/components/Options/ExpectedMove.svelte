          ...
          {#each rawData as item, index}
            {#if data?.user?.tier === "Pro" || index === 0}
              ...
              >
                ...
                <span class="ml-2 text-xs text-fg">
      ...
      <div class="flex flex-row items-center gap-4">
        ...
        <DropdownMenu.Root>
          <DropdownMenu.Trigger asChild let:builder>
            <Button
              builders={[builder]}
              class="w-fit sm:w-auto transition-all duration-150 border border-line text-fg bg-surface-card hover:bg-[#f8fbfb] dark:hover:bg-zinc-900/70 flex flex-row justify-between items-center px-2 sm:px-3 py-2 rounded-full truncate"
            >

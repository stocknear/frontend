...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  if (!["Plus", "Pro"]?.includes(user?.tier)) {
    return new Response(
      JSON.stringify({
        error:
          "This feature is available exclusively for Subscribers. Please upgrade your plan to access insider trading summaries.",
      }),
  ...
  try {
    const response = await fetch(apiURL + "/insider-summary", {
      method: "POST",
      headers: {

<script lang="ts">
  ...
  async function popularStrategy(state: string) {
    ...
    ruleOfList?.forEach((row) => {
      ...
      if (checkedRules.includes(row.name) && Array.isArray(row.value)) {
        checkedItems.set(
          row.name,
          new Set(normalizeCheckedRuleValues(row.value)),
        );
      }
    });
  ...
  onMount(async () => {
    ...
    if (
      !hasInitialFeed ||
      rowsPerPage !== (data?.getScreenerFeed?.pageSize ?? 20)
    ) {
        ...
        <div class="mt-3 pb-1 w-full">
          ...
          {#if data?.user}
            <label
              for={!data?.user ? "userLogin" : ""}
              on:click={() => handleSave(true)}
              class="text-[0.95rem] sm:ml-3 cursor-pointer inline-flex items-center justify-center space-x-1 whitespace-nowrap rounded-control border border-gray-300 dark:border-none bg-blue-brand_light py-2 pl-3 pr-4 font-semibold  bg-white sm:hover:bg-gray-100 dark:bg-[#000] dark:sm:hover:bg-default/60 ease-out focus:outline-hidden "
            >
              <svg
                class="h-5 w-5"
                xmlns="http://www.w3.org/2000/svg"
                    ...
                    {#if column.key === "symbol"}
                      <td class=" whitespace-nowrap text-start">
                        <a
                          ...
                          target="_blank"
                          class="font-medium text-fg transition-colors hover:text-accent text-sm sm:text-[0.95rem]"
                          >{item?.symbol}</a
                        >
                      </td>

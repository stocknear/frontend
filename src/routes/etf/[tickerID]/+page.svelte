<script lang="ts">
  ...
  function historicalPrice(timePeriod: string): Promise<boolean> {
    const ticker = normalizeTicker($etfTicker);
    ...
    const isCurrent = () =>
      ...
      ticker === normalizeTicker($etfTicker) &&
...
<SEO
  title={`${data?.companyName} (${$etfTicker}) ETF Price, Holdings & Performance`}
  description={`Analyze ${data?.companyName} (${$etfTicker}) with holdings, expense ratio, assets under management, dividend yield and long-term performance. Track portfolio diversification and compare the fund against its peers.`}
    ...
    >
      ...
      <div class="pb-12 md:pb-20 w-full sm:pr-6 xl:pr-0">
        <div class="mt-2">
          ...
          >
            <div
              class="order-1 lg:order-5 grow overflow-hidden border-line py-0.5 xs:py-1 sm:px-0.5 sm:pb-3 sm:pt-2.5 lg:mb-0 lg:border-0 lg:border-l lg:px-0 lg:py-0 lg:pl-5 md:mb-4 md:border-b"
            >
          ...
          {displayData === interval
                              ...
                              : 'bg-transparent text-fg-muted hover:text-accent hover:bg-gray-100/60 dark:hover:bg-zinc-900/60'}"
                          >
                            {$screenWidth < 1200
                              ? interval
            ...
            >
              <table class="w-[50%] text-sm lg:min-w-[250px]">
                <tbody
                  ><tr
                    class="flex flex-col border-b border-line min-h-[60px] justify-center sm:table-row sm:min-h-0"
                    ><td

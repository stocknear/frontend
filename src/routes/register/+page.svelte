  ...
  <div class="mx-auto max-w-lg px-4 sm:px-6 py-8 sm:py-16">
    ...
    {#if currentStep === 1}
      ...
      <div>
        ...
        <p class="text-xs text-center text-gray-500 dark:text-zinc-400 mt-4">
          {register_terms_prefix()}
          <a href="/terms-of-use" class="text-gray-700 dark:text-zinc-300 hover:text-violet-500 transition">{register_terms_link()}</a>
          {register_terms_middle()}
          <a href="/privacy-policy" class="text-gray-700 dark:text-zinc-300 hover:text-violet-500 transition">{register_privacy_link()}</a>.
        </p>

        <!-- Sign in link -->
        <p class="text-sm text-center text-gray-500 dark:text-zinc-400 mt-6">
          {register_has_account()}
          <a href="/login" class="text-violet-700 dark:text-violet-400 hover:underline ml-1">{register_signin_link()}</a>
        </p>
      </div>
    ...
    {:else if currentStep === 2}
      ...
      <div>
        ...
        <div class="mb-8 flex flex-wrap items-center justify-center gap-3 text-sm font-semibold">
          ...
          <label class="relative inline-flex cursor-pointer items-center">
            ...
            <span class="h-8 w-16 rounded-full border border-gray-300 dark:border-zinc-600 transition {pricingAnnual ? 'bg-emerald-500' : 'bg-gray-200 dark:bg-zinc-800'}"></span>
            <span class="absolute left-1 top-1 h-6 w-6 rounded-full bg-white shadow-md transition peer-checked:translate-x-8"></span>
          </label>
        ...
        <div class="grid gap-6 sm:grid-cols-2">
          ...
          <div class="rounded-2xl border border-gray-200 dark:border-zinc-700 bg-white dark:bg-zinc-900/60 p-6 flex flex-col">
            ...
            <div class="mt-4 flex items-baseline gap-2">
              {#if pricingAnnual}
                <span class="text-lg text-gray-400 dark:text-zinc-500 line-through">$10</span>
                <span class="text-3xl font-semibold text-gray-900 dark:text-white">$5</span>
              {:else}
                <span class="text-3xl font-semibold text-gray-900 dark:text-white">$15</span>
              {/if}
              <span class="text-sm text-gray-500 dark:text-zinc-400">{register_step2_per_month()}</span>
            </div>
            {#if pricingAnnual}
              <p class="mt-1 text-xs text-gray-500 dark:text-zinc-500">
                {register_step2_billed_annually()} (<span class="line-through">$120</span> $60)
              </p>
            {/if}
            <ul class="mt-5 mb-5 space-y-2 text-sm flex-1">
              {#each [pricing_feature_credits_150(), pricing_feature_watchlist_unlimited(), pricing_feature_portfolio_unlimited(), pricing_feature_alerts_unlimited(), pricing_feature_screener_unlimited(), pricing_feature_download_unlimited(), pricing_feature_notification(), pricing_feature_hedgefund(), pricing_feature_congress(), pricing_feature_no_ads()] as feature}
                <li class="flex items-start gap-2">
                  <svg class="w-4 h-4 mt-0.5 flex-shrink-0 text-gray-600 dark:text-zinc-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="rounded-2xl border-2 border-violet-500 dark:border-violet-400 bg-white dark:bg-zinc-900/60 p-6 flex flex-col relative">
            ...
            <div class="mt-4 flex items-baseline gap-2">
              {#if pricingAnnual}
                <span class="text-lg text-gray-400 dark:text-zinc-500 line-through">$30</span>
                <span class="text-3xl font-semibold text-gray-900 dark:text-white">$15</span>
              {:else}
                <span class="text-3xl font-semibold text-gray-900 dark:text-white">$45</span>
              {/if}
              <span class="text-sm text-gray-500 dark:text-zinc-400">{register_step2_per_month()}</span>
            </div>
            {#if pricingAnnual}
              <p class="mt-1 text-xs text-gray-500 dark:text-zinc-500">
                {register_step2_billed_annually()} (<span class="line-through">$360</span> $180)
              </p>
            {/if}
            <ul class="mt-5 mb-5 space-y-2 text-sm flex-1">

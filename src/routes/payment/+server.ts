...
function billingProofHeaders(recordId, tier, lifetime) {
  ...
  return {
    ...
    "X-Stocknear-Billing-Signature": signature,
  };
}
...
export const POST = async ({ request, locals }) => {
  try {
    ...
    if (eventName === "subscription_expired" && userId) {
      try {
        ...
        await locals.pb.collection("users").update(
          ...
          { headers: billingProofHeaders(userId, tier, lifetime) },
        );
    ...
    if (eventName === "order_created") {
      ...
      const lifetime = productName?.includes("Life Time") ?? false;

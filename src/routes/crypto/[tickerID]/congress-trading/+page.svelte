<script lang="ts">
  ...
  import { displayCompanyName, numberOfUnreadNotification, cryptoTicker } from '$lib/store';
...
<svelte:head>
  ...
  <title>
    {$numberOfUnreadNotification > 0 ? `(${$numberOfUnreadNotification})` : ''} {$displayCompanyName} ({$cryptoTicker}) US Congress & Senate Trading · stocknear
  </title>
  <meta name="description" content={`Get the latest US congress & senate trading of ${$displayCompanyName} (${$cryptoTicker}) from democrates and republicans.`} />

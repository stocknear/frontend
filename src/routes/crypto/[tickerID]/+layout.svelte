...
async function toggleUserWatchlist(watchListId: string) {
  ...
  try {
    ...
    if (watchlistIndex !== -1) {
      const existingTickerIndex = userWatchList[watchlistIndex]?.ticker?.indexOf($cryptoTicker);
...
$: {
  if ($cryptoTicker && $cryptoTicker?.length !== 0 && typeof window !== 'undefined') // add a check to see if running on client-side
  {
...
$: {
  if(userWatchList)
  {
    isTickerIncluded = userWatchList?.some(item => item.user === data?.user?.id && item?.ticker?.includes($cryptoTicker));
  }
}
                ...
                <div class="flex-1 flex-shrink-0 flex flex-row items-center justify-between -mt-2">
                    ...
                    <div class="{!isScrolled ? 'hidden' : 'flex flex-col items-center ml-6 transition-transform ease-in'}">
                      ...
                      <label on:click={loadSearchData} class="ml-auto mr-4" for="searchBarModal">
                       ...
                       <!--Start Share Button-->
                      <label class="mr-4" on:click={() => shareContent('https://stocknear.com/crypto/'+$cryptoTicker)} >
    ...
    <div class= "flex flex-col">
      <div class="text-white text-md flex flex-col flex-shrink-0">
          <div class="rounded-full w-10 h-10 relative bg-gray-900 mb-2">
              <img class="rounded-full w-6 h-6 absolute inset-1/2 transform -translate-x-1/2 -translate-y-1/2" src={`https://financialmodelingprep.com/image-stock/${$cryptoTicker}.png`} />
          </div>
          <span class="mb-1">
    ...
    <ul class="menu dropdown-content text-white bg-[#000] rounded -ml-6">
      <li class="mb-3">
        <label for="typeOfTrade" on:click={() => handleTypeOfTrade('buy')} class="cursor-pointer flex flex-row justify-start items-center">
          ...
          <span class="ml-1 text-white text-lg font-medium">Buy {$cryptoTicker} Shares</span>

<script lang="ts">
  ...
  async function processReachedAlertsOnServer(): Promise<void> {
    ...
    try {
      ...
      const triggeredCount =
        ...
        0;
      const triggeredSymbols = Array.isArray(payload?.triggeredSymbols)
        ? payload.triggeredSymbols
            .map((symbol) =>
              String(symbol || "")
                .trim()
                .toUpperCase(),
  ...
  async function handleDeleteTickers() {
    ...
    earnings = earnings?.filter(
      (item) => !symbolsToDelete.includes(item?.symbol),
    );
  ...
  onMount(() => {
    ...
    enqueueReachedAlertsForServerSync(priceAlertList);
    priceAlertAudio = new Audio("/audio/notification.mp3");
                        ...
                        >
                          {#if toTarget.valid}
                            <div class="min-w-[130px]">
                              ...
                              >
                                <span
                                  class="text-xs font-medium {toTarget.textClass}"
                                >
                                  {toTarget.label}
                                </span>
                                <span

...
<SEO
  title={hedge_funds_seo_title()}
  description={hedge_funds_seo_description()}
  keywords={hedge_funds_seo_keywords()}
  structuredData={{
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: hedge_funds_structured_name(),
    description: hedge_funds_structured_description(),
        ...
        >
          <main class="w-full">
            ...
            <div class="w-full m-auto mt-5">
              <div
                class="w-full m-auto mb-4 rounded-container border border-line bg-surface-card overflow-x-auto relative transition-opacity duration-200"
                      ...
                      {#each tableData as item, index}
                        ...
                        >
                          {#each columns as column}
                            {#if column.key === "rank"}
                              <td
                                class="text-[0.85rem] sm:text-sm text-center text-fg tabular-nums"
                              >
                                {item?.rank}
                              </td>

<script lang="ts">
  ...
  import ChartModal from "$lib/components/CashSecuredPut/ChartModal.svelte";
  import BreadCrumb from "$lib/components/BreadCrumb.svelte";

  import {
    cash_secured_put_screener_breadcrumb_home,
    cash_secured_put_screener_breadcrumb_title,
    cash_secured_put_screener_seo_title,
    cash_secured_put_screener_seo_description,
    cash_secured_put_screener_seo_keywords,
    cash_secured_put_screener_structured_name,
    cash_secured_put_screener_structured_description,
    ...
    cash_secured_put_screener_select_popular,
    cash_secured_put_screener_saved_screens,
    cash_secured_put_screener_select_screen,
    cash_secured_put_screener_new_screen,
    cash_secured_put_screener_strategy_high_yield,
    cash_secured_put_screener_strategy_conservative,
    cash_secured_put_screener_strategy_weekly_income,
    cash_secured_put_screener_strategy_high_protection,
    cash_secured_put_screener_strategy_bluechip_csp,
    cash_secured_put_screener_filters_count,
    cash_secured_put_screener_add_filters,
    cash_secured_put_screener_search_filters,
    cash_secured_put_screener_select_filters_title,
    cash_secured_put_screener_nothing_found,
    cash_secured_put_screener_save,
    cash_secured_put_screener_save_as_new,
    cash_secured_put_screener_reset_all,
    cash_secured_put_screener_condition_over,
    cash_secured_put_screener_condition_under,
    cash_secured_put_screener_condition_between,
    cash_secured_put_screener_condition_exactly,
    cash_secured_put_screener_condition_any,
    cash_secured_put_screener_input_min,
    cash_secured_put_screener_input_max,
    cash_secured_put_screener_input_value,
    cash_secured_put_screener_search_placeholder,
    cash_secured_put_screener_search_modal_placeholder,
    cash_secured_put_screener_normal_width,
    cash_secured_put_screener_full_width,
    cash_secured_put_screener_tab_general,
    cash_secured_put_screener_tab_income,
    cash_secured_put_screener_tab_filters,
    cash_secured_put_screener_tab_greeks,
    cash_secured_put_screener_pagination_previous,
    cash_secured_put_screener_pagination_next,
    cash_secured_put_screener_pagination_page_of,
    cash_secured_put_screener_rows_label,
    cash_secured_put_screener_back_to_top,
    cash_secured_put_screener_no_results_query,
    cash_secured_put_screener_no_contracts,
    cash_secured_put_screener_modal_new_title,
    cash_secured_put_screener_modal_new_name_label,
    cash_secured_put_screener_modal_new_create,
    cash_secured_put_screener_modal_delete_title,
    cash_secured_put_screener_modal_delete_message,
    cash_secured_put_screener_modal_delete_cancel,
    cash_secured_put_screener_modal_delete_confirm,
    cash_secured_put_screener_toast_deleting,
    cash_secured_put_screener_toast_deleted,
    cash_secured_put_screener_toast_delete_failed,
    cash_secured_put_screener_toast_title_too_long,
    cash_secured_put_screener_toast_created,
    cash_secured_put_screener_toast_creating,
    cash_secured_put_screener_toast_create_failed,
    cash_secured_put_screener_toast_select_rule,
    cash_secured_put_screener_toast_saving,
    cash_secured_put_screener_toast_saved,
  ...
  const popularStrategyList = [
    ...
    {
      key: "weeklyIncome",
      label: cash_secured_put_screener_strategy_weekly_income(),
    },
    {
      key: "highProtection",
  ...
  function conditionLabel(condition) {
    switch (condition) {
      case "over":
        return cash_secured_put_screener_condition_over();
      case "under":
        return cash_secured_put_screener_condition_under();
      case "between":
        return cash_secured_put_screener_condition_between();
      case "exactly":
        return cash_secured_put_screener_condition_exactly();
      default:
        return condition ?? "";
    }
  }
  ...
  async function handleDeleteStrategy() {
    ...
    return toast?.promise(deletePromise, {
      loading: cash_secured_put_screener_toast_deleting(),
      success: cash_secured_put_screener_toast_deleted(),
      error: cash_secured_put_screener_toast_delete_failed(),
      style: `
          border-radius: 5px;
          background: #fff;
          color: #000;
  ...
  async function createStrategy(event) {
    ...
    if (title?.length > 100) {
      toast.error(cash_secured_put_screener_toast_title_too_long(), {
        style: `
          border-radius: 5px;
          background: #fff;
          color: #000;
    ...
    const postData = { type: "cashSecuredPutScreener" };
    ...
    const createPromise = (async () => {
      ...
      toast.success(cash_secured_put_screener_toast_created(), {
        style: `
          border-radius: 5px;
          background: #fff;
          color: #000;
    ...
    return toast.promise(createPromise, {
      loading: cash_secured_put_screener_toast_creating(),
      success: () => "",
      error: cash_secured_put_screener_toast_create_failed(),
      style: `
          border-radius: 5px;
          background: #fff;
          color: #000;
  ...
  async function popularStrategy(state: string) {
    ...
    const strategies = {
      ...
      conservative: {
        name: "Conservative",
        rules: [
          { condition: "under", name: "moneynessPercent", value: "-5%" },
          { condition: "over", name: "delta", value: "-0.3" },
      ...
      bluechipCSP: {
        name: "Blue-Chip Cash Secured Puts",
        rules: [
  ...
  function handleAddRule() {
    if (ruleName === "") {
      toast.error(cash_secured_put_screener_toast_select_rule(), {
  ...
  function saveRowsPerPage() {
    ...
    try {
      const paginationKey = `cash-secured-put-screener_rowsPerPage`;
  ...
  function toggleFullWidth() {
    ...
    try {
      localStorage.setItem(
        "cash-secured-put-screener-full-width",
        String(isFullWidth),
      );
    } catch (e) {
    ...
    if (strategyList?.length > 0) {
      ...
      if (showMessage) {
        return toast.promise(savePromise, {
          loading: cash_secured_put_screener_toast_saving(),
          success: cash_secured_put_screener_toast_saved(),
          error: cash_secured_put_screener_toast_save_failed(),
          style: `
              border-radius: 5px;
              background: #fff;
              color: #000;
  ...
  function getColumnOrderStorageKey(): string {
    return `cash-secured-put-screener_${displayTableTab}_columnOrder`;
  }
    ...
    if (displayTableTab) {
      ...
      const baseSortOrdersMap = {
        income: {
          ...
          discount: { order: "none", type: "number" },
...
<SEO
  title={cash_secured_put_screener_seo_title()}
  description={cash_secured_put_screener_seo_description()}
  keywords={cash_secured_put_screener_seo_keywords()}
  structuredData={{
    "@context": "https://schema.org",
    "@type": "SoftwareApplication",
    name: cash_secured_put_screener_structured_name(),
    description: cash_secured_put_screener_structured_description(),
    url: "https://stocknear.com/cash-secured-put-screener",
...
>
          ...
          <div class="relative inline-block text-left grow">
            <DropdownMenu.Root>
                  ...
                  <span class="truncate"
                    >{selectedPopularStrategy?.length !== 0
                      ? (popularStrategyList.find(
                        ...
                        )?.label ?? cash_secured_put_screener_select_popular())
                      : cash_secured_put_screener_select_popular()}</span
                  >
                  <svg
              ...
              <DropdownMenu.Content
                ...
                sideOffset={10}
                alignOffset={0}
                class="w-fit h-fit max-h-72 overflow-y-auto scroller rounded-container border border-line bg-surface-card p-1.5 text-fg shadow-none"
              >
                <DropdownMenu.Label
        ...
        <div class="flex w-full sm:w-[50%] sm:ml-3 md:block md:w-auto ml-3">
          ...
          <div class="relative inline-block text-left grow">
            <DropdownMenu.Root>
                ...
                >
                  <span class="truncate w-full"
                    >{selectedStrategy?.length !== 0
                      ...
                      : cash_secured_put_screener_select_screen()}</span
                  >
                  <svg
                    class="-mr-1 ml-1 h-5 w-5 xs:ml-2 inline-block"
                    viewBox="0 0 20 20"
              ...
              <DropdownMenu.Content
                class="w-fit  max-w-52 h-fit max-h-72 overflow-hidden overflow-y-auto scroller rounded-container border border-line bg-surface-card p-1.5 text-fg shadow-none"
              >
                <DropdownMenu.Label
                  class="text-fg font-normal"
                >
      ...
      {#if showFilters}
        <div class="mt-3 pb-1 w-full">
          <div class="flex flex-wrap items-center gap-2.5">
            ...
            <label
              for="ruleModal"
              class="w-full sm:w-fit text-sm inline-flex cursor-pointer items-center justify-center space-x-1 whitespace-nowrap rounded-full border border-line/90 dark:border-white/80 bg-gray-900 text-white dark:bg-white dark:text-gray-900 py-2 pl-3 pr-4 font-semibold transition hover:bg-gray-800 dark:hover:bg-zinc-200 focus:outline-hidden"
            >
              <svg
                class="h-5 w-5"
                viewBox="0 0 20 20"
              ...
              <div>{cash_secured_put_screener_add_filters()}</div>
            </label>
            ...
            <div class="w-full sm:w-fit relative">
              <div class="relative">
                ...
                <input
                  type="text"
                  placeholder={cash_secured_put_screener_search_filters({
            ...
            {#if data?.user}
              <label
                ...
                class="w-full sm:w-fit text-sm cursor-pointer inline-flex items-center justify-center space-x-1 whitespace-nowrap rounded-full border border-line/90 dark:border-white/80 bg-gray-900 text-white dark:bg-white dark:text-gray-900 py-2 pl-3 pr-4 font-semibold transition hover:bg-gray-800 dark:hover:bg-zinc-200 focus:outline-hidden"
              >
                <svg
                ...
                <div>{cash_secured_put_screener_save()}</div>
              </label>
            ...
            {#if ruleOfList?.length !== 0}
              ...
              >
                ...
                <div>{cash_secured_put_screener_reset_all()}</div>
              </label>
            {/if}
          </div>
        </div>
                  ...
                  <div on:click={() => (ruleName = row?.rule)}>
                    <DropdownMenu.Root>
                          ...
                          <span class="truncate ml-2 text-sm">
                            ...
                            {:else if ruleCondition[row?.rule] === "between"}
                              {Array.isArray(valueMappings[row?.rule])
                                ? `${valueMappings[row?.rule][0]}-${valueMappings[row?.rule][1] ?? cash_secured_put_screener_condition_any()}`
                                : cash_secured_put_screener_condition_any()}
                      ...
                      <DropdownMenu.Content
                        ...
                        sideOffset={10}
                        alignOffset={0}
                        class="h-fit max-h-72 overflow-hidden overflow-y-auto scroller rounded-container border border-line bg-surface-card p-2 text-fg shadow-none {row?.rule ===
                            ...
                            >
                              ...
                              {#if ruleCondition[row?.rule] === "between"}
                                <div class="flex gap-x-1 -ml-2 z-10 -mt-1">
                                  <input
                                    type="text"
                                    placeholder={cash_secured_put_screener_input_min()}
                                    value={Array?.isArray(
                                      valueMappings[row?.rule],
                                    )
                                  ...
                                  <input
                                    type="text"
                                    placeholder={cash_secured_put_screener_input_max()}
                                    value={Array?.isArray(
                                      valueMappings[row?.rule],
                                    )
                              ...
                              {:else}
                                <input
                                  type="text"
                                  placeholder={cash_secured_put_screener_input_value()}
  ...
  >
    <h2
      class=" whitespace-nowrap type-h2 text-fg py-1 bp:text-[1.3rem] border-line text-fg"
    >
      ...
      >
        ...
        >
          ...
          <span class="truncate text-[0.85rem] sm:text-sm"
            >{isFullWidth
              ? cash_secured_put_screener_normal_width()
              : cash_secured_put_screener_full_width()}</span
          >
        </button>
            ...
            {#each displayedData as item, i}
              ...
              >
                {#each columns as column}
                  ...
                  {:else if column.key === "annualizedReturn" || column.key === "discount" || column.key === "downsideProtection" || column.key === "iv" || column.key === "ivRank" || column.key === "pctBeBid" || column.key === "returnVal" || column.key === "profitProb"}
                    <td class="text-end text-sm whitespace-nowrap">
          ...
          <div class="flex items-center gap-2">
            ...
            >
              ...
              <span class="hidden sm:inline"
                >{cash_secured_put_screener_pagination_previous()}</span
              >
            </Button>
          </div>
    ...
    {:else}
      <Infobox
        text={inputValue?.length > 0
          ? cash_secured_put_screener_no_results_query({ query: inputValue })
          : cash_secured_put_screener_no_contracts()}
      />
    {/if}
  {:else}
  ...
  >
    ...
    >
      <Input
        ...
        type="text"
        errors=""
        label={cash_secured_put_screener_modal_new_name_label()}
        required={true}
      />
      <button
        type="submit"
    ...
    <div class="flex justify-end space-x-3">
      ...
      ><label
        ...
        ><svg
          ...
          ></line></svg
        >
        {cash_secured_put_screener_modal_delete_confirm()}</label
      >
    </div>
  </div>
</dialog>

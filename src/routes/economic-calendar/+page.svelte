...
<section class="w-full max-w-4xl overflow-hidden m-auto min-h-screen pt-5 pb-40">
          ...
          <div class="w-full max-w-4xl m-auto sm:bg-[#202020] sm:rounded-xl h-auto sm:p-10 mt-10 sm:mt-3 mb-8">
            ...
            <div class="flex justify-center w-full max-w-5xl m-auto h-full overflow-hidden">
                ...
                <div class=" w-full flex flex-row justify-center m-auto items-center pl-2 pr-2 sm:pl-0 sm:pr-0">
                    <!-- Start Columns -->
                    <label on:click={() => changeWeek('previous')} class="{previousMax ? 'opacity-80' : ''} hidden sm:flex h-16 w-48 cursor-pointer border m-auto flex bg-[#3C40F0] hover:bg-blue-700 border border-blue-600 mb-3">
                    ...
                    <div class="w-full {index === selectedWeekday ? '' : 'hidden sm:block'}">
                            <label on:click={() => toggleDate(index)} class="w-11/12 m-auto sm:w-full cursor-pointer h-16 {index === selectedWeekday ? 'bg-[#3C74D4] bg-opacity-[0.6] sm:bg-[#A24D51] sm:gradient-effect' : ''} rounded-lg sm:rounded-none flex bg-[#3C40F0] border border-blue-600 mb-3">
                    ...
                    {/each}
                    <label on:click={() => changeWeek('next')} class="{nextMax ? 'opacity-80' : ''} hidden sm:flex h-16 w-48 cursor-pointer border m-auto flex bg-[#3C40F0] hover:bg-blue-700 border border-blue-600 mb-3">
...
{#if filterList?.length !== 0}
<label on:click={() => filterList = [] } class="mr-3 text-sm cursor-pointer bg-[#202020] sm:hover:bg-[#202020] duratiion-100 transition ease-in-out px-4 py-2 rounded-lg shadow-lg ml-auto">
...
<div class="dropdown dropdown-end z-30">
  <button tabindex="0" role="button" class="text-sm cursor-pointer bg-[#202020] sm:hover:bg-[#202020] duratiion-100 transition ease-in-out px-4 py-2 rounded-lg shadow-lg ml-auto">
    Filter
                  ...
                  {#if index === selectedWeekday}
                    {#if day?.length !== 0}
                      <table class="hidden sm:inline-table table-sm table-compact rounded-none sm:rounded-md w-full border-bg-[#0F0F0F] m-auto mt-4 ">
                          ...
                          <td class="{item?.impact === 'Low' ? 'text-[#00FC50]' : item?.impact === 'Medium' ? 'text-[#3DDBFE]' : item?.impact === 'High' ? 'text-[#FC2120]' : 'text-white'} text-sm text-end mr-1 border-b-[#0F0F0F]">
                            {item?.impact}
                          </td>

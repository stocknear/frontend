<script lang="ts">
  ...
  const createChartRangeSelector = (chart: any) => {
    const state = {
      active: false,
      dragging: false,
    ...
    const updateOverlay = () => {
      ...
      const rangeText =
        isFiniteNumber(t1) && isFiniteNumber(t2)
          ? `${formatRangeTimestamp(t1, displayData)}–${formatRangeTimestamp(
              t2,
              displayData,
            )}`
          : "";
    ...
    const onPointerMove = (evt: PointerEvent) => {
      ...
      if (
        ...
        Math.abs(chartX - state.startChartX) > 6
      ) {
        state.active = true;
      }
    ...
    const stopDrag = (evt: PointerEvent) => {
      if (!state.dragging || state.pointerId !== evt.pointerId) return;
    ...
    chart.container.addEventListener("pointerup", stopDrag);
    chart.container.addEventListener("pointercancel", stopDrag);
    ...
    return {
      sync(range: string) {
        if (state.lastRange && state.lastRange !== range) {
          clear();
        }
          ...
          {displayData === interval
            ...
            >
              <table class="w-[50%] text-sm sm:text-[1rem] lg:min-w-[250px]">
                <tbody
                  ><tr
                    class="flex flex-col border-b border-gray-300 dark:border-gray-800 min-h-[60px] justify-center sm:table-row sm:min-h-0"
                    ><td

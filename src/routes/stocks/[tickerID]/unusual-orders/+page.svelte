...
<SEO
  title={stock_detail_unusual_orders_seo_title({
    company: $displayCompanyName,
    ticker: $stockTicker,
  })}
  description={stock_detail_unusual_orders_seo_description({
    company: $displayCompanyName,
    ticker: $stockTicker,
  })}
  keywords={stock_detail_unusual_orders_seo_keywords({
    ticker: $stockTicker,
    company: $displayCompanyName,
  })}
  structuredData={{
    ...
    headline: stock_detail_unusual_orders_structured_headline({
      company: $displayCompanyName,
    }),
    description: stock_detail_unusual_orders_structured_desc({
      company: $displayCompanyName,
      ticker: $stockTicker,
    }),
...
<section class="w-full overflow-hidden h-full">
  <div class="w-full flex h-full overflow-hidden">
    ...
    >
      <div class="sm:pl-7 sm:pb-7 sm:pt-3 pt-5 w-full m-auto">
        <div class="w-full">
          {#if priceLevel?.length === 0 && hottestTrades?.length === 0 && historicalDarkPool?.length === 0}
            <Infobox
              text={stock_detail_unusual_orders_no_activity({
                company: $displayCompanyName,
              })}
            />
          {/if}

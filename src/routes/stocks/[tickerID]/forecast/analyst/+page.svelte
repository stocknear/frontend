  ...
  <div class="flex justify-center m-auto h-full overflow-hidden w-full">
    ...
    >
      <div class="sm:pl-4 sm:pt-4 w-full m-auto mt-2 sm:mt-0">
        ...
        <div
          class="shadow mb-4 grid grid-cols-2 grid-rows-2 divide-contrast rounded border border-gray-300 dark:border-gray-600 md:grid-cols-4 md:grid-rows-1 md:divide-x"
        >
          <div
                          ...
                          {#if Math?.ceil(item?.adjusted_pt_current) !== 0 && data?.getStockQuote?.price}
                            {@const upsideValue =
                              (item?.adjusted_pt_current /
                                ...
                                100 || 0}
                            <span
                              class="text-sm sm:text-[1rem] whitespace-nowrap {upsideValue >=
                              0
                        ...
                        {:else if column.key === "date"}
                          <div class="flex flex-col items-end">
                            {#if latestInfoDate(item?.date)}
                              <label
                                class="border-gray-300 shadow bg-black dark:bg-[#fff] text-white dark:text-black font-semibold text-xs rounded px-2 py-0.5 mb-1"
                              >
                                New
                              </label>
                            {/if}
            ...
            >
              <div class="flex items-center gap-2">
                <Button
                  ...
                  class="w-fit transition-all flex flex-row items-center duration-50 border border-gray-300 dark:border-gray-700 text-white bg-black sm:hover:bg-default dark:bg-primary dark:sm:hover:bg-secondary sm:w-auto px-1.5 sm:px-3 rounded"
                >
                  <svg
